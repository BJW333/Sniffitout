...
def packetcallback(packet):
    ...
    text = f"Source: {ip_src}|Destination: {ip_dst}|Protocol: {ip_protocol}|Number: {ip_proto}\n"

    
    results_text.insert(tk.INSERT, text)
    
    #IPv4 and IPv6 parts
    start_idx = '1.0'
    while True:
        start_idx = results_text.search(ip_src, start_idx, stopindex=tk.END)
        if not start_idx:
            break
        end_idx = f"{start_idx}+{len(ip_src)}c"
        if ':' in ip_src:  # If it is IPv6
            results_text.tag_add('light_purple', start_idx, end_idx)
        else:  # If it is IPv4
            results_text.tag_add('light_blue', start_idx, end_idx)
        start_idx = end_idx

    #destionation
    start_idx = '1.0'
    while True:
        start_idx = results_text.search(ip_dst, start_idx, stopindex=tk.END)
        if not start_idx:
            break
        end_idx = f"{start_idx}+{len(ip_dst)}c"
        if ':' in ip_dst:  # If it is IPv6
            results_text.tag_add('light_purple', start_idx, end_idx)
        else:  # If it is IPv4
            results_text.tag_add('light_blue', start_idx, end_idx)
        start_idx = end_idx
    ...
    start_idx = '1.0'
    while True:
        start_idx = results_text.search(ip_protocol, start_idx, stopindex=tk.END)
        if not start_idx:
            break
        end_idx = f"{start_idx}+{len(ip_protocol)}c"
        if ip_protocol:  # If it is IPv6
            results_text.tag_add('yellow', start_idx, end_idx)
        start_idx = end_idx
...
root.geometry("1050x550")  # 800x600Set initial window size
...
results_text = scrolledtext.ScrolledText(resultsframe, width=115, height=40)
results_text.pack(side=tk.TOP, expand=True)
results_text.pack(side=tk.RIGHT)

results_text.tag_config('light_blue', foreground='#33b9ff')
results_text.tag_config('light_purple', foreground='#9370DB')
results_text.tag_config('yellow', foreground='#FFE36E')
